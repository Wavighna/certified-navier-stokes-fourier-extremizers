"""One-command proof verification for the public static-extremizer release.
...
PROOFS = ROOT / "proofs"
CANDIDATE = PROOFS / "static_adaptive_kkt_candidate.json"
HIGH_BRANCH_CANDIDATE = PROOFS / "high_branch_eta10000_candidate.json"
...
def verify() -> dict[str, object]:
    ...
    high_branch_payload = certify_high_static_branch(
        ...
        PROOFS / "high_branch_eta10000_arb_certificate.json",
        (256, 512),
    )
    ...
    return {
        ...
        "inputs": {
            ...
            "inviscid_six_local_certificate": _record(
                PROOFS / "inviscid_six_full64_local_certificate.json"
            ),
            "inviscid_competing_local_certificate": _record(
                PROOFS / "inviscid_competing_full64_local_certificate.json"
            ),
            "uniform_high_eta_six_branch": _record(
                PROOFS / "six_branch_high_eta_uniform.json"
            ),
            "shell112_sos_certificate": _record(
                PROOFS / "shell112_sos_certificate.json"
            ),
            "shell123_sos_certificate": _record(
                PROOFS / "shell123_sos_certificate.json"
            ),
            "shell224_sos_certificate": _record(
                PROOFS / "shell224_sos_certificate.json"
            ),
            "shell334_sos_certificate": _record(
                PROOFS / "shell334_sos_certificate.json"
            ),
            "high_branch_reduced_formula": _record(
                PROOFS / "reduced_competing_formula.json"
            ),
            "competing_symmetry_group": _record(
                PROOFS / "competing_symmetry_group.json"
            ),
            "uniform_high_eta_competing": _record(
                PROOFS / "competing_high_eta_uniform.json"
            ),
            "uniform_all_eta_competing": _record(
                PROOFS / "competing_all_eta_uniform.json"
            ),
            "symmetry_branch_crossover": _record(
                PROOFS / "symmetry_branch_crossover.json"
            ),
            "competing_hessian_spectral_blocks": _record(
                PROOFS / "competing_hessian_spectral_blocks.json"
            ),
            "competing_all_eta_symbolic_continuation": _record(
                PROOFS / "competing_all_eta_symbolic_continuation.json"
            ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", default="proofs/Wuthoo_2026_Verification_Summary.recomputed.json"
    )
